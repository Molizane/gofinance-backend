...
const getAccountsByUserIdAndTypeAndCategoryIdAndDescription = `-- name: GetAccountsByUserIdAndTypeAndCategoryIdAndDescription :many
SELECT a.id, a.user_id, a.title, a.type, a.description, a.value, a.date, a.created_at, c.title as category_title
FROM accounts a
LEFT JOIN categories c ON c.id = a.category_id
WHERE a.user_id = $1
...
type GetAccountsByUserIdAndTypeAndCategoryIdAndDescriptionParams struct {
	UserID      int32  `json:"user_id"`
	Type        string `json:"type"`
	CategoryID  int32  `json:"category_id"`
	Description string `json:"description"`
...
type GetAccountsByUserIdAndTypeAndCategoryIdAndDescriptionRow struct {
	ID            int32          `json:"id"`
	UserID        int32          `json:"user_id"`
	Title         string         `json:"title"`
	Type          string         `json:"type"`
...
func (q *Queries) GetAccountsByUserIdAndTypeAndCategoryIdAndDescription(ctx context.Context, arg GetAccountsByUserIdAndTypeAndCategoryIdAndDescriptionParams) ([]GetAccountsByUserIdAndTypeAndCategoryIdAndDescriptionRow, error) {
	rows, err := q.db.QueryContext(ctx, getAccountsByUserIdAndTypeAndCategoryIdAndDescription,
		arg.UserID,
		arg.Type,
		arg.CategoryID,
		arg.Description,
	...
	defer rows.Close()
	items := []GetAccountsByUserIdAndTypeAndCategoryIdAndDescriptionRow{}
	for rows.Next() {
		var i GetAccountsByUserIdAndTypeAndCategoryIdAndDescriptionRow
		if err := rows.Scan(
			&i.ID,
			&i.UserID,
			&i.Title,
...
AND a.type = $2
AND a.category_id = $3
AND LOWER(a.title) LIKE CONCAT('%', LOWER($4::text), '%')
`
...
AND a.category_id = $3
AND LOWER(a.title) LIKE CONCAT('%', LOWER($4::text), '%')
AND LOWER(a.description) LIKE CONCAT('%', LOWER($5::text), '%')
`
...
WHERE a.user_id = $1
AND a.type = $2
AND LOWER(a.description) LIKE CONCAT('%', LOWER($3::text), '%')
`
...
func (q *Queries) GetAccountsReports(ctx context.Context, arg GetAccountsReportsParams) (interface{}, error) {
	row := q.db.QueryRowContext(ctx, getAccountsReports, arg.UserID, arg.Type)
	var sum_value interface{}
